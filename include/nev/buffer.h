...
class NEV_EXPORT Buffer {
 public:
  ...
  explicit Buffer(size_t initialSize = kInitialSize)
    ...
    DCHECK(writableBytes() == initialSize);
  ...
  void appendInt64(int64_t x) {
    int64_t be64 = base::HostToNet64(x);
  ...
  void appendInt32(int32_t x) {
    int32_t be32 = base::HostToNet32(x);
  ...
  ssize_t readFd(SocketDescriptor fd, int* savedErrno);
 ...
 private:
  ...
  void makeSpace(size_t len) {
    if (writableBytes() + prependableBytes() < len + kCheapPrepend) {
      ...
      DCHECK(kCheapPrepend < readerIndex_);
